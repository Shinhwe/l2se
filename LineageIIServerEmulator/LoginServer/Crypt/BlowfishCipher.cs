...
namespace LineageIIServerEmulator.LoginServer.Crypt
{
    public class BlowfishCipher
    {
        ...
        /// </summary>
        private static uint[] SBOX_INIT_0 =
        {
        ...
        /// </summary>
        private static uint[] SBOX_INIT_1 =
        {
        ...
        /// </summary>
        private static uint[] SBOX_INIT_2 =
        {
        ...
        /// </summary>
        private static uint[] SBOX_INIT_3 =
        {
        ...
        public byte[] Key
        {
            ...
            set
            {
                ...
                Array.Copy(SBOX_INIT_0, 0, this.sBoxes[0], 0, 256);
                Array.Copy(SBOX_INIT_1, 0, this.sBoxes[1], 0, 256);
                Array.Copy(SBOX_INIT_2, 0, this.sBoxes[2], 0, 256);
                Array.Copy(SBOX_INIT_3, 0, this.sBoxes[3], 0, 256);
        ...
        /// </summary>
        /// <param name="capacity">The key.</param>
        public BlowfishCipher(byte[] key)
        {
            this.pBox = new uint[18];
            this.sBoxes = new uint[4][];

            for (int i = 0; i < 4; i++)
            {
                this.sBoxes[i] = new uint[256];
            }
        ...
        private void InitializeArrays()
        {
            ...
            for (int i = 0; i < 18; i++)
            {
                ...
                this.pBox[i] = BlowfishCipher.PBOX_INIT[i] ^ data;
            }
            ...
            for (int i = 0; i < 18; i += 2)
            {
                ...
                this.pBox[i] = BitConverter.ToUInt32(b, 0);
                this.pBox[i + 1] = BitConverter.ToUInt32(b, 4);
            }

            for (int i = 0; i < 4; i++)
            {
                this.InitializeSBox(b, this.sBoxes[i]);
            }
        }
        ...
        private void InitializeSBox(byte[] b, uint[] sBox)
        {
        ...
        public void Encrypt(byte[] data, int offset, int length)
        {
            ...
            for (int k = 0; k < blockNumber; k++)
            {
                ...
                uint xl = BitConverter.ToUInt32(data, p);
                uint xr = BitConverter.ToUInt32(data, p + 4);
                uint tmp;
        ...
        public void Decrypt(byte[] data, int offset, int length)
        {
            ...
            for (int k = 0; k < blocks; k++)
            {
                ...
                uint lb = BitConverter.ToUInt32(data, p);
                uint rb = BitConverter.ToUInt32(data, p + 4);
                uint tmp;
        ...
        private uint F(uint x)
        {
            ...
            uint y = this.sBoxes[0][a] + this.sBoxes[1][b];
